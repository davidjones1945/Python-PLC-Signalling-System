...
class App(QMainWindow): #hlavná triedy vizualizácie 
    def __init__(self, parent=None):
        ...
        self.vlaknoUpdate.dataUpdated.connect(lambda: self.szz.stavanieCesty(True))
        ...
        self.zoznamNav[9].zhasnute = True
    ...
    def update(self, ID:int=-1, clicked:bool=False, objekt:str='update'):    #metóda pre aktualizáciu symbolov objektov
        ...
        if objekt in ['update', 'navestidla']:  #aktualizácia návestidiel
            ...
            self.zoznamNav[9].zhasnute =  not self.vlaknoUpdate.dictTS[1].smer
    ...
    def clickObjekt(self, id:int, objekt:str):  #metóda spracovávajúca kliknutie na objekt
        if objekt == 'navestidlo':
            ...
            if not self.vlaknoUpdate.dictStanice[1].dialkove:   #ak má precovisko aktívne riadenie 
    ...
    def akciaNavestidlo(self, index:int, typ:str):   #vyhodnotenie vybranej akcie z kontextového menu
        ...
        if ((typ in ['hlavne', 'kombi']) and index == 1):    #výber vlakovej cesty
            if (self.zoznamNav[self.posledneNav].nazov not in ['R_k1_fik','R_k2_fik']):  
        ...
        elif ((typ == 'kombi' and index == 2) or (typ == 'zriad' and index == 1)):
            if (self.zoznamNav[self.posledneNav].nazov in ['R_Se1','R_L1','R_L2']):  #výber posunovej cesty
                if self.zoznamNav[self.posledneNav].znak != 'PN':
        ...
        #---------------------------stavanie jazdnej cesty------------------------------------------------------------------------
            ...
            if self.zoznamNav[self.pociatocneNav].nazov in ['R_S']:
                self.postavCestu()
            ...
            if self.zoznamNav[self.pociatocneNav].nazov in ['R_L1','R_L2']:
        ...
        elif (typ == 'fikt' and index == 1) or (typ == 'kombi' and index == 4) or (typ == 'zriad' and index == 2):    #rušenie cesty
        ...
        elif (typ == 'hlavne' and index == 2) or (typ == 'kombi' and index == 3): 
            if self.zoznamNav[self.posledneNav].nazov not in ['R_k1_fik','R_k2_fik']:   #kontrola správneho návestidla 
        ...
        elif (typ == 'hlavne' and index == 5) or (typ == 'kombi' and index == 6) or (typ == 'zriad' and index == 4):
            if self.zoznamNav[self.posledneNav].nazov not in ['R_k1_fik','R_k2_fik']:
                self.zoznamNav[self.posledneNav].vybrane = False

                if self.zoznamNav[self.posledneNav].pociatocne:   #iba ak je návestidlo počiatočným návestidlom jazdnej cesty
            ...
            if self.zoznamNav[self.posledneNav].nazov not in ['R_k1_fik','R_k2_fik']:
                if self.zoznamNav[self.posledneNav].znak != 'PN':
                    ...
                    if self.zoznamNav[self.posledneNav].pociatocne:   #iba ak je návestidlo počiatočným návestidlom jazdnej cesty
                        ...
                        if (typ == 'hlavne') or (typ == 'kombi' and not self.zoznamNav[self.posledneNav].typAktCes):
    ...
    def akciaTS(self, index:int):   #metóda pre prácu s traťovým súhlasom
        if (index == 1):   #žiadosť o TS    
            if  (not self.vlaknoUpdate.dictTS[1].prijem) and (not self.vlaknoUpdate.dictTS[1].odchod):
                if self.vlaknoUpdate.dictTS[1].volnost: 
        ...
        elif (index == 3) and (self.vlaknoUpdate.dictTS[1].ziadost is True):   #udelenie TS
    ...
    def akciaStanica(self, index:int):  #metóda pre spracovanie signálov riadenia
        if index == 1: #žiadosť o prevzatie riadenia
            if self.workerThread.dictStanice[self.poslednaStn].dialkove:
                if self.workerThread.dictStanice[self.poslednaStn].ziadost: #ak už je aktívna žiadosť
            ...
            if self.workerThread.dictStanice[self.poslednaStn].ziadost:
